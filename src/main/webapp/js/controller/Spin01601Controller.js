...
var dtoSpin01601 = null;

wmsController.controller('Spin01601Ctrl', ['$scope', '$http', '$location', '$modal', '$rootScope', '$timeout', 'AlertService'
	...
	function fnInit(){
		...
		if(dtoSpin01601 == null || dtoSpin01601.initFlg == true){
			dtoSpin01601 = new spin01601Dto();
		}

		if(dtoSpin01601.initFlg == false){
			// 検索
			fnSearchRecords(dtoSpin01601.pageNum, tmt030System.SYSNUMVAL2, true);
			dtoSpin01601.initFlg = true;
		}

		// 保持している検索条件を画面にセット
    	$timeout(function() {
    		if(dtoSpin01601.arvlplndateFrom == undefined || dtoSpin01601.arvlplndateFrom == ""){
    			$scope.$broadcast ('setDate1', datePickerInitTypeBlank);
    		} else {
    			$scope.$broadcast ('setDate1', datePickerInitTypeValue, dtoSpin01601.arvlplndateFrom);
    		}

    		if(dtoSpin01601.arvlplndateTo == undefined || dtoSpin01601.arvlplndateTo == ""){
    			$scope.$broadcast ('setDate2', datePickerInitTypeBlank);
    		} else {
    			$scope.$broadcast ('setDate2', datePickerInitTypeValue, dtoSpin01601.arvlplndateTo);
    		}
		});

    	$scope.siplnno = dtoSpin01601.siplnno;
		$scope.stscd = dtoSpin01601.stscd;
		$scope.divkbnModel = dtoSpin01601.divkbn;
		$scope.itemcd = dtoSpin01601.itemcd;
		$scope.itemnm = dtoSpin01601.itemnm;
		$scope.spplycd = dtoSpin01601.spplycd;
		$scope.spplynm = dtoSpin01601.spplynm;

		// 初期表示Webサービス
		var res = callWebService(service + spin01601InitWs, request, function(response) {}, null, null, null);
	}
    ...
    function fnSearchRecords(pageNum, dispNum, initFlg){
        ...
        var errFlg = false;
        var resRec = callWebService(service + spin01601SearchCntWs, request,
            function(response) {
        		console.log("success");
...
/*		arvlplndateFrom = $scope.datetimeValue.date1;
		...
		dtoSpin01601.arvlplndateFrom = $scope.datetimeValue.date1;
		dtoSpin01601.arvlplndateTo = $scope.datetimeValue.date2;
		dtoSpin01601.siplnno = $scope.siplnno;
		dtoSpin01601.stscd = $scope.stscd;
		dtoSpin01601.divkbn = $scope.divkbnModel;
		dtoSpin01601.itemcd = $scope.itemcd;
		dtoSpin01601.spplycd = $scope.spplycd;
		dtoSpin01601.spplynm = $scope.spplynm;
	}

    // リクエストを取得する
    function fnGetRequest(pageNum, dispNum){
    	var request = {
            	"spin01601SearchCondition":{
                	"ARVLPLNDATEFROM": dtoSpin01601.arvlplndateFrom,
                    "ARVLPLNDATETO": dtoSpin01601.arvlplndateTo,
                    "SIPLNNO": dtoSpin01601.siplnno,
                    "STSCD": dtoSpin01601.stscd,
                    "DIVKBN": dtoSpin01601.divkbn,
                    "ITEMCD": dtoSpin01601.itemcd,
                    "SPPLYCD": dtoSpin01601.spplycd
                },
                "pageInfo":{
                	"pageNum": pageNum,
                    "dispNum": dispNum
    ...
    function fnGetRecords(pageNum, dispNum){
    	...
    	var request = fnGetRequest(pageNum, dispNum);
 		...
 		var res = callWebService(service + spin01601SearchWs, request,
	        function(response) {
