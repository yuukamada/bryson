 ...
 * 概要：検索
 */
public class Spin00301SearchAllRecProcess extends AbstractProcess {

	public Spin00301SearchAllRecProcess(ILogSender logSender) {
		super(logSender);
	}
	...
	public void beforeProcess(DBAccessor dba, AbstractRequest request, AbstractResponse response, AbstractResponse parentResponse) throws ProcessCheckErrorException {
		...
		Spin00301SearchRequest searchItemInfo = (Spin00301SearchRequest) request;
		String arvlplndateFrom = searchItemInfo.spin00301SearchCondition.ARVLPLNDATEFROM;
		String arvlplndateTo = searchItemInfo.spin00301SearchCondition.ARVLPLNDATETO;
	...
	//PDF出力
	public String createPdf(Spin00301SearchResponse res) throws DocumentException, IOException{
		...
		//表の要素(列タイトル)を作成
		PdfPCell cell1_1 = new PdfPCell(new Paragraph("入荷予定日", font));
		cell1_1.setGrayFill(0.8f);		//セルを灰色に設定
		table.addCell(cell1_1);
		PdfPCell cell1_2 = new PdfPCell(new Paragraph("入荷伝票番号", font));
		...
		if(res.rows != null)
		{
			for(int i = 0; i < res.rows.size(); i++){
				Spin00301SearchRowDto dto = res.rows.get(i);
				PdfPCell cellX_1;
				cellX_1 = new PdfPCell(new Paragraph(dto.ARVLPLNDATE, font));
				PdfPCell cellX_2 = new PdfPCell(new Paragraph(dto.SIPLNNO, font));
				//表の要素を追加する
				table.addCell(cellX_1);
				table.addCell(cellX_2);
			}
		}
	...
	public AbstractResponse process(DBAccessor dba, AbstractRequest request, AbstractResponse response, AbstractResponse parentResponse)
		...
		try {
			...
			String arvlplndateFrom = searchItemInfo.spin00301SearchCondition.ARVLPLNDATEFROM;
			String arvlplndateTo = searchItemInfo.spin00301SearchCondition.ARVLPLNDATETO;
			String sipnno = searchItemInfo.spin00301SearchCondition.SIPLNNO;
			String stscd = searchItemInfo.spin00301SearchCondition.STSCD;
			String divkbn = searchItemInfo.spin00301SearchCondition.DIVKBN;
			String itemcd = searchItemInfo.spin00301SearchCondition.ITEMCD;
			String spplycd = searchItemInfo.spin00301SearchCondition.SPPLYCD;
			...
			// 実行結果をレスポンスにつめる
			Spin00301SearchResponse resSpin00301Search = (Spin00301SearchResponse)response;
			setResponse(rs, resSpin00301Search);

			try {
				createPdf(resSpin00301Search);
			...
			// 1件もレコードがなかったらエラーにする
			if (resSpin00301Search.rows != null && resSpin00301Search.rows.size() == 0) {
				resSpin00301Search.rows = null;
			}

			return resSpin00301Search;
	...
	{
		ArrayList<Spin00301SearchRowDto> lst = new ArrayList<Spin00301SearchRowDto>();

		while (rs.next()) {
			Spin00301SearchRowDto row = new Spin00301SearchRowDto();
